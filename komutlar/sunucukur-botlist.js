  ...
  message.channel
    ...
    .then(resulter => {
      ...
      yes.on("collect", async reaction => {
        ...
        message.guild.roles
          .create({ data: { name: "Kurucu" }, reason: "ayn" })
          .then(role => {
        ...
        message.guild.roles
          .create({ data: { name: "Yönetici" }, reason: "ayn" })
          .then(role => {
            role.setPermissions([
              "MANAGE_GUILD",
              "MANAGE_ROLES",
        ...
        message.guild.channels
          .create("📌・BİLGİLENDİRME", { type: "category" })
          .then(parent => {
            message.guild.channels
            ...
            message.guild.channels
              .create("🚀・boost", { type: "text" })
              .then(c => c.setParent(parent.id));
          });

        message.guild.channels
          .create("📌・GENEL", { type: "category" })
          .then(parent => {
            message.guild.channels
              .create("🗨️・sohbet", { type: "text" })
              .then(c => c.setParent(parent.id));
        ...
        message.guild.channels
          .create("📌・BOT-LIST", { type: "category" })
          .then(parent => {
            message.guild.channels
              .create("🤖・bot-kuralları", { type: "text" })
              .then(a => a.setParent(parent.id));

            message.guild.channels
              .create("🤖・bot-ekle", { type: "text" })
              .then(a => a.setParent(parent.id));

            message.guild.channels
              .create("🤖・bot-log", { type: "text" })
              .then(a => a.setParent(parent.id));

            message.guild.channels
              .create("🤖・bot-test", { type: "text" })
              .then(a => a.setParent(parent.id));
          });

        message.guild.channels
          .create("📌・LOG", { type: "category" })
          .then(parent => {
            message.guild.channels
              .create("🗳️・sayaç", { type: "text" })
              .then(c => c.setParent(parent.id));

            message.guild.channels
              .create("📊・giriş-cıkış", { type: "text" })
              .then(c => c.setParent(parent.id));
          });
      });
