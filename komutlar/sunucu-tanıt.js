...
exports.run = async (client, message, args) => {
  ...
  if (lastDaily !== null && cooldown - (Date.now() - lastDaily) > 0) {
    ...
    const pinkcode = new Discord.MessageEmbed()
      .setDescription(
        "Bu Komutu Sadece `24 Saatte` Bir Kullanabilirsin <a:tik1:798276995011051530>"
      )
      .setColor(0x00ffff);
    return message.channel.send({ pinkcode });
  } else {
    const pinkcode = new Discord.MessageEmbed()
      .setTitle("Başarılı <a:tik1:798276995011051530>")
      .setDescription(
        "**Sunucunuz Başarıyla** [Sunucumda](https://discord.gg/w8GYuBqZ) **Tanıtıldı.**\n**24 Saat Sonra Tekrar Sunucunuzu Tanıtabilirsiniz <a:dikkatalert:797756800798818305>**"
      )
      .setColor("GREEN");
    message.channel.send(pinkcode);
    message.channel.createInvite({ maxAge: 0 }).then(invite => {
      const pinkcode2 = new Discord.MessageEmbed()
      ...
      client.channels.cache.get("924257213965541417").send(pinkcode2);
